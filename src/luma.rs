...
use {Alpha, Xyz, Yxy};
use {Blend, Component, ComponentWise, FromColor, Limited, Mix, Pixel, Shade};
use white_point::{D65, WhitePoint};
use {cast, clamp};
use blend::PreAlpha;

/// Linear luminance with an alpha component. See the [`Lumaa` implementation
...
where
    ...
    Wp: WhitePoint,
{
    fn clone(&self) -> Luma<Wp, T> {
        *self
    }
}

unsafe impl<Wp: WhitePoint, T: Component + Float> Pixel<T> for Luma<Wp, T> {
    ...
    T: Component + Float,
{
    ...
    pub fn new_u8(luma: u8, alpha: u8) -> Lumaa<D65, T> {
        Alpha {
            ...
            alpha: cast::<T, _>(alpha) / cast(255.0),
        }
    }
}
...
mod test {
    ...
    raw_pixel_conversion_tests!(Luma<D65>: luma);
}
