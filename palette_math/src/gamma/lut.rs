...
pub unsafe trait GammaLutOutput: gamma_lut_output_seal::Sealed {
    ...
    /// Look up a value of this output type.
    fn lookup(
        linear: f32,
        linear_slope: f32,
        min_float_bits: u32,
    ...
    E: GammaLutOutput,
{
    ...
    ///
    /// This function is primarily meant for code generation.
    ///
    /// # Safety
    ///
    /// The parts need to come from a [`GammaLutBuilder`] and be valid for the
    /// linear and encoded types.
...
#[inline]
fn linear_to_encoded_u8(linear: f32, min_float_bits: u32, table: &[u32]) -> u8 {
    ...
    unsafe { linear_float_to_encoded_uint(input, min_float_bits, table) }
}

#[inline]
fn linear_to_encoded_u16(
    linear: f32,
    linear_slope: f32,
    min_float_bits: u32,
    table: &[u64],
